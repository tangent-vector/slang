...
//
// Everything in a fossil file is encoded in units of 32-bit words,
// and thus everything should land on a 4-byte-aligned boundary.
//

using FossilInt = int32_t;
using FossilUInt = uint32_t;

/// Alignment for all fields/objects/data in a fossil-format blob.
static const int kFossilAlignment = 4;
...
//
// For simple types that are 8 bytes in size, we fossilize
// them with only 4-byte alignment, so an unaligned load
// is required to get their value.
//

template<typename T, FossilizedValKind Kind>
struct alignas(uint32_t) FossilizedUnalignedSimpleVal
{
public:
    using Layout = FossilizedValLayout;
    static const FossilizedValKind kKind = Kind;
    ...
    {
        T result;
        ::memcpy(&result, _valueData, sizeof(result));
        return result;
    }
...
private:
    char _valueData[sizeof(T)];
};

#define SLANG_DECLARE_FOSSILIZED_UNALIGNED_SIMPLE_TYPE(TYPE, TAG)                           \
    template<>                                                                    \
    struct FossilizedTypeTraits<TYPE>                                             \
    {                                                                             \
        using FossilizedType = FossilizedUnalignedSimpleVal<TYPE, FossilizedValKind::TAG>; \
    };

SLANG_DECLARE_FOSSILIZED_UNALIGNED_SIMPLE_TYPE(int64_t, Int64)
SLANG_DECLARE_FOSSILIZED_UNALIGNED_SIMPLE_TYPE(uint64_t, UInt64)
SLANG_DECLARE_FOSSILIZED_UNALIGNED_SIMPLE_TYPE(double, Float64)
...
static_assert(alignof(Fossilized<int64_t>) == 4);
static_assert(alignof(Fossilized<uint64_t>) == 4);
static_assert(alignof(Fossilized<double>) == 4);



//
// Simple scalars that are smaller than 4 bytes will
// always be fossilized as a comparable 4-byte type.
//
...
#define SLANG_DECLARE_FOSSILIZED_VIA_CAST(TYPE, FOSSILIZED_AS)              \
    template<>                                                              \
    struct Fossilized_<TYPE> : FossilizedViaCastVal<TYPE, FOSSILIZED_AS>    \
    {                                                                       \
    };

SLANG_DECLARE_FOSSILIZED_VIA_CAST(int8_t, int32_t)
SLANG_DECLARE_FOSSILIZED_VIA_CAST(int16_t, int32_t)

SLANG_DECLARE_FOSSILIZED_VIA_CAST(bool, uint32_t)
SLANG_DECLARE_FOSSILIZED_VIA_CAST(uint8_t, uint32_t)
SLANG_DECLARE_FOSSILIZED_VIA_CAST(uint16_t, uint32_t)

static_assert(sizeof(Fossilized<int8_t>) == 4);
static_assert(sizeof(Fossilized<int16_t>) == 4);

static_assert(sizeof(Fossilized<bool>) == 4);
static_assert(sizeof(Fossilized<uint8_t>) == 4);
static_assert(sizeof(Fossilized<uint16_t>) == 4);
...
///
template<typename T>
SLANG_FORCE_INLINE ValPtr<T> as_assumingNonNull(AnyValPtr valPtr)
{
    SLANG_ASSERT(valPtr != nullptr);
    if (!T::isMatchingKind(valPtr->getKind()))
    {
        return nullptr;
    }
...
SLANG_FORCE_INLINE Fossil::AnyValRef Fossil::ValRef<FossilizedContainerObjBase>::getElement(
    Index index) const
{
...
SLANG_FORCE_INLINE FossilizedRecordElementLayout* FossilizedRecordLayout::getField(
    Index index) const
{
    SLANG_ASSERT(index >= 0);
...
SLANG_FORCE_INLINE Fossil::AnyValRef Fossil::ValRef<FossilizedRecordVal>::getField(
    Index index) const
{
    SLANG_ASSERT(index >= 0);
