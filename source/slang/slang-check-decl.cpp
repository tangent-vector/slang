    ...
    RefPtr<WitnessTable> witnessTable)
{
    ...
    if (auto existingDecl = context->parentDecl->findFirstDirectMemberOfName(requirementDeclRef.getName()))
    {
...
//    parentDecl->buildMemberDictionary();
    for (auto oldDecl = parentDecl->findNextDirectMemberDeclWithSameName(newDecl);
        oldDecl;
...
void SemanticsDeclBasesVisitor::_validateExtensionDeclMembers(ExtensionDecl* decl)
{
    for (auto ctor : decl->getMembersOfType<ConstructorDecl>())
    {
        if (!ctor->body || ctor->getDirectMemberDeclCount() != 0)
            continue;
        getSink()->diagnose(ctor->loc, Diagnostics::invalidMemberTypeInExtension, ctor->astNodeType);
    }
}
...
void SemanticsDeclHeaderVisitor::visitIncludeDecl(IncludeDecl* decl)
{
    ...
    if (auto implementing = as<ImplementingDecl>(firstMember))
    {
        ...
        {
            if (auto moduleDeclarationDecl = as<ModuleDeclarationDecl>(moduleDecl->getDirectMemberDecl(0)))
            {
