...
// This file implements conversion of the Slang IR to
// the "bytecode" format.
...
struct IRToBCNodeBuilder : RefObject
{
    /// The IR value that is being lowered into bytecode here
    IRParentInst*   irValue;

    /// The type of this node, as a local ID in the parent...
    LocalID typeID;
    ...
    /// child nodes
    List<RefPtr<IRToBCNodeBuilder>> children;

    BCWriteOffsetBase writeData(BCWriter const& inWriter)
    {
        BCWriter writer = inWriter;

        auto bcNode = writer.reserve<SlangBCIRNode>();
        writer.setBaseOffset(bcNode);

        auto registerCount = registers.Count();
        auto bcRegisters = writer.reserve<BCReg>(registerCount);

        auto blockCount = blocks.Count();
        auto bcBlocks = writer.reserve<BCBlock>(blockCount);

        auto upValueCount = constants.Count();
        auto bcUpValues = writer.reserve<SlangBCUpValue>(upValueCount);

        auto childCount = children.Count();
        auto bcChildOffsets = writer.reserve<uint32_t>(childCount);

        auto codeSize = currentBytecode.Count();
        auto bcCode = writer.reserve<SlangBCCode>(codeSize);

        bcNode->op      = irValue->op;
        bcNode->typeID  = typeID;
        bcNode->registerCount = registerCount;
        bcNode->registersOffset = bcRegisters.offset;
        bcNode->blockCount = blockCount;
        bcNode->blocksOffset = bcBlocks.offset;
        bcNode->upValueCount = upValueCount;
        bcNode->upValuesOffset = bcUpValues.offset;
        bcNode->nodeCount = childCount;
        bcNode->nodesOffset = bcChildOffsets.offset;
        bcNode->codeSize = codeSize;
        bcNode->codeOffset = bcCode.offset;

        for(UInt ii = 0; ii < registerCount; ++ii)
            bcRegisters[ii] = registers[ii];
        for(UInt ii = 0; ii < blockCount; ++ii)
            bcBlocks[ii] = blocks[ii];
        for(UInt ii = 0; ii < upValueCount; ++ii)
            bcUpValues[ii] = constants[ii];
        for(UInt ii = 0; ii < codeSize; ++ii)
            bcCode[ii] = currentBytecode[ii];

        for(UInt ii = 0; ii < childCount; ++ii)
        {
            auto childNode = children[ii]->writeData(writer);
            bcChildOffsets[ii] = childNode - bcNode;
        }
...
struct IRSection : BCSectionBuilder
{
    IRSection()
    {
        this->type = SLANG_BC_SECTION_TYPE_IR;
    }

    List<RefPtr<IRToBCNodeBuilder>> nodes;
    ...
    void writeData(BCWriter& writer)
    {
        ...
        auto bcSectionHeader = writer.reserve<SlangBCIRSectionHeader>();
        auto bcNodeOffsets = writer.reserve<NodeTableEntry>(nodeCount);

        bcSectionHeader->nodeCount = nodeCount;
        bcSectionHeader->nodeTableEntrySize = sizeof(NodeTableEntry);
        bcSectionHeader->nodeTableOffset = bcNodeOffsets.offset;
...
struct SharedIRToBCContext
{
    RefPtr<IRSection> irSection;
};

struct IRToBCContext
{
    /// Shared state for encoding all values to BC
    SharedIRToBCContext*    shared;

    /// The node we are building
    IRToBCNodeBuilder*  node;
...
void encodeUInt8(
    ...
    uint8_t         value)
{
    context->node->currentBytecode.Add(value);
}
...
void encodeUInt(
    ...
    UInt             value)
{
    if(value < 0x80) // 8 - 1 = 7 bits
    {
        UInt code = value << 1;
        assert(code < 0x100);
        encodeUInt8(context, (uint8_t)code);
        return;
    }
    else if(value < 0x4000) // 16 - 2 = 14 bits
    {
        UInt code = (value << 2) | 1;
        assert(code < 0x10000);
        encodeUInt8(context, (uint8_t)(code & 0xFF));
        encodeUInt8(context, (uint8_t)((code >> 8) & 0xFF));
    }
    else if(value < 0x20000000) // 32 - 3 = 29 bits
    {
        uint32_t code = (value << 3) | 3;
        assert((code >> 3) == value);
        encodeUInt8(context, (uint8_t)(code & 0xFF));
        encodeUInt8(context, (uint8_t)((code >> 8) & 0xFF));
        encodeUInt8(context, (uint8_t)((code >> 16) & 0xFF));
        encodeUInt8(context, (uint8_t)((code >> 24) & 0xFF));
    }
    else
    {
        SLANG_UNEXPECTED("IR operand too large");
    }
}
    ...
    IRInst*         value)
{
    ...
    if(auto parent = context->parent)
    {
        ...
        UInt constantIndex = context->node->constants.Count();
    ...
    IRInst*         inst)
{
    ...
    case kIROp_GenericKind:
        {
            // Operations with no explicit type, but with an explicit destination
            encodeUInt(context, inst->op);
            encodeOperand(context, inst);
        }
        break;
...
void generateBytecodeNodeForIRParentRec(
    SharedIRToBCContext*    sharedContext,
    IRToBCContext*          parentContext,
    IRParentInst*           inst)
{
    RefPtr<IRToBCNodeBuilder> node = new IRToBCNodeBuilder();

    if(parentContext)
    {
        parentContext->node->children.Add(node);
    }
    else
    {
        sharedContext->irSection->nodes.Add(node);
    }
    ...
    {
        node->typeID = getLocalID(parentContext, type);
    }
    ...
    if(auto code = as<IRGlobalValueWithCode>(inst))
    {
        ...
        node->blocks.SetSize(blockCount);
        ...
        for( auto bb = code->getFirstBlock(); bb; bb = bb->getNextBlock() )
        {
            ...
            node->blocks[blockID].paramCount = (uint32_t)paramCount;
        }
        ...
        node->registers.SetSize(regCount);
        ...
        for( auto bb = code->getFirstBlock(); bb; bb = bb->getNextBlock() )
        {
            ...
            //
            node->blocks[blockID].firstParamIndex = (uint32_t)regCounter;
            for( auto ii = bb->getFirstInst(); ii; ii = ii->getNextInst() )
            {
                switch(ii->op)
                {
                default:
                    ...
                    if( opHasResult(ii) )
                    {
                        ...
                        node->registers[localID].op = ii->op;
                        node->registers[localID].previousVarIndexPlusOne = (uint32_t)localID;
                        node->registers[localID].typeID = getLocalID(context, ii->getFullType());
                    }
                    break;
                ...
                case kIROp_Var:
                    ...
                    // will be the storage for the variable value.
                    {
                        ...
                        node->registers[localID+1].op = ii->op;
                        node->registers[localID+1].previousVarIndexPlusOne = (uint32_t)localID+1;
                        node->registers[localID+1].typeID = getLocalID(context,
        ...
        for( auto bb = code->getFirstBlock(); bb; bb = bb->getNextBlock() )
        {
            ...
            UInt codeOffset = node->currentBytecode.Count();
            node->blocks[blockID].codeOffset = codeOffset;
    ...
    else
    {
        ...
        for( auto ii = inst->getFirstChild(); ii; ii = ii->getNextInst() )
        {
            ...
            node->registers[localID].typeID = ii->getFullType() ? getLocalID(context, ii->getFullType()) : 0;
        }
        ...
        // We need to terminate the block with an instruction just
        // to note that we've reached the end.
        encodeUInt(context, kIROp_End);
    }
    ...
    for( auto ii = inst->getFirstChild(); ii; ii = ii->getNextInst() )
    {
        ...
        {
            generateBytecodeNodeForIRParentRec(sharedContext, context, parentInst);
        }
    }
}
...
{
    auto irSection = fileBuilder->addSection<IRSection>(".ir");

    SharedIRToBCContext sharedContextStorage;
    auto sharedContext = &sharedContextStorage;
    sharedContext->irSection = irSection;

    // There might be multiple translation units in the compile request,
    // each of which will map to a distinct IR-level module.
    for (auto translationUnitReq : compileRequest->translationUnits)
    {
        generateBytecodeNodeForIRParentRec(sharedContext, nullptr, translationUnitReq->irModule->getModuleInst());
    }
}
