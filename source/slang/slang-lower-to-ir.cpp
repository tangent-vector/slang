...
struct ExprLoweringContext
{
    ...
    void _lowerSubstitutionEnv(IRGenContext* subContext, DeclRefBase* subst)
    {
        ...
        if (auto genSubst = as<GenericAppDeclRef>(subst))
        {
            ...
            Index argCounter = 0;
            for (auto memberDecl : genDecl->getMembers())
            {
...
#undef IGNORED_CASE
    ...
    LoweredValInfo visitInterfaceDecl(InterfaceDecl* decl)
    {
        ...
        for (auto requirementDecl : decl->getMembers())
        {
            ...
            {
                for (auto accessorDecl : as<ContainerDecl>(requirementDecl)->getMembers())
                {
        ...
        for (auto requirementDecl : decl->getMembers())
        {
            ...
            if (!requirementKey)
            {
                ...
                {
                    for (auto member : as<ContainerDecl>(requirementDecl)->getMembers())
                    {
...
static void ensureAllDeclsRec(IRGenContext* context, Decl* decl)
{
    ...
    {
        for (auto memberDecl : fileDecl->getMembers())
        {
            ensureAllDeclsRec(context, memberDecl);
        }
    }
